...
export default {
    ...
    mounted(){
        this.$axios.get(`/catalogos/municipios/${this.id}`).then( ({data}) => this.objeto = data)
    },
    methods:{
        actualizarObjeto(){
            this.$axios.put(`/catalogos/municipios/${this.id}`, this.objeto).then( () => this.$router.push('/catalogos/municipios'))
            .catch( e => {
