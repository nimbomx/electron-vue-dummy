...
export default {
    ...
    mounted(){
        this.$axios.get(`/catalogos/zonas/${this.id}`).then( ({data}) => this.objeto = data)
    },
    methods:{
        actualizarObjeto(){
            this.$axios.put(`/catalogos/zonas/${this.id}`, this.objeto).then( () => this.$router.push('/catalogos/municipios'))
            .catch( e => {
